...
Def: Purpose → Execute hierarchical code review for Rust source files against registered standards.
...
- Rule: HierarchicalReadingSequence: Enforce sequential reading of registered standard rules before code inspection:
  ...
  4. `41-lang-standard-rust`

Step: 2. Target Scope Resolution:
- Rule: TargetFilter: Resolve review targets from `[target]` (or uncommitted git changes); filter strictly to Rust extensions (`*.rs`).
