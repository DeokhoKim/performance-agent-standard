...
SHARED_RULES=(
  ...
  "42|lang-standard-cpp|glob|*.cpp, *.cc, *.cxx, *.hpp, *.h, *.hxx|Read when writing, reviewing, or optimizing C++ source code (*.cpp, *.hpp, etc.). Enforces memory safety, deleted copy operations, RAII smart pointers, pre-allocated C arrays, template restraint, lock-free atomic design, and Unix/Android optimization."
  "43|lang-standard-cpp-17|glob|*.cpp, *.cc, *.cxx, *.hpp, *.h, *.hxx|Read when targeting C++17 projects. Enforces std::string_view pass-by-value, structured bindings, if constexpr, sum-type error handling, SFINAE constraints, and PMR monotonic stack arenas."
  "44|lang-standard-cpp-20|glob|*.cpp, *.cc, *.cxx, *.hpp, *.h, *.hxx|Read when targeting C++20 projects. Enforces concepts over SFINAE, std::span native views, spaceship comparison (<=>), designated initializers, std::jthread cooperative cancellation, and std::ranges/views."
  "45|lang-standard-cpp-23|glob|*.cpp, *.cc, *.cxx, *.hpp, *.h, *.hxx|Read when targeting C++23 projects. Enforces std::expected monadic error handling, std::flat_map, std::mdspan, deducing this, std::print, and std::unreachable."
